...
Metody:
    ...
    Pridanie primary key pri tvorbe tabuľky:
        primarykey(
            ...
            data_type->boolean, (pri vlastnom dátovom typom) || prednastavená hodnota="INT"
    ...
    Pridanie stlpca pri tvorbe tabuľky alebo pridanie stlpca do už existujucej tabuľky:
        column(
            "name"->string,
            "data_type"->string,
            NotNull->boolean, prednastavená hodnota=false (honoty môžu byť NULL)
            Unique->boolean, prednastavená hodnota=false
            "default"->string, prednastavená hodnota=""
            "addtoexisttable"->string, (pridá daný stlpec do existujucej tabuľky) || prednastavená hodnota=""
        );
    ...
    Uplné a nenavrátné odstranenie tabuľky aj napriek foreign key:
        droptable(
            "nazov"->string,
        );
    ...
    Vloženie udajov do tabuľky:
        insert(
            ...
            [data]->array, (pre vloženie viacerých udajov do jedneho riadka) || "column"->string (pre jeden udaj) || [[data]]-> 2D array (pre vloženie viac riadkov)
            );
    ...
    Zoskupenie riadkov, ktoré majú rovnaké hodnoty, do súhrnných riadkov, použivané pri agregačných funckiach 
...
class SQL
{
    ...
    //Pridavanie stlpcou pri vytvaraný novej tabuľky
    ...
    //Pridavanie foreign key pri vytvaraný novej tabuľky
    ...
    //Zisťovanie, ako treba upraviť data aby boly v tvare pre sql
    ...
    //Zisťovanie, ako treba upraviť stlpce aby boly v tvare pre sql
